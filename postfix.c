...
void toPostfix()
{
	...
	while(*exp != '\0')
	{
		...
		// 이후 스택에서 한 번 더 데이터를 빼내어 왼쪽 괄호 (를 버리고, 그대로 다음 문자를 읽어 오른쪽 괄호 )도 버립니다.
			...
			while(x != '\0' && token1 <= getPriority(x)){
				x = postfixPop();				// 불러온 연산자의 우선순위가 스택에서 가져온 연산자의 우선순위보다 낮다면 스택에서 가져온 연산자를 하나씩 꺼내 후위 표기식에 추가하고, 조건을 만족하는 스택에서의 연산자가 없다면 불러온 연산자를 스택에 추가합니다. 이후 다음 문자를 불러옵니다.
				charCat(&x);
