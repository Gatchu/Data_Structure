...
void toPostfix()
{
	...
	while(*exp != '\0')
	{
		...
		else if (token1 > getPriority(x)){					// 불러온 연산자의 우선순위가 스택에서 가져온 연산자의 우선순위보다 높다면 스택에서 가져온 연산자를 스택에 그대로 두고, 불러온 연산자를 스택에 저장하고 다음 문자를 불러옵니다.
				postfixPush(*exp);
				exp++;
		}
		else if(token1 <= getPriority(x)){					// 불러온 연산자의 우선순위가 스택에서 가져온 연산자의 우선순위보다 낮다면 스택에서 가져온 연산자를 꺼내 후위 표기식에 추가하고 불러온 연산자를 스택에 넣습니다. 이후 다음 문자를 불러옵니다.
				charCat(&x);
				postfixPop();
				postfixPush(*exp++);
		}
